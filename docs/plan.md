...
- [ ] `asm::lexer`：标识符、数字字面量（10/16/2 进制，带 `H/B` 后缀）、字符与字符串常量、寄存器名、标点、注释（`;`）。
- [ ] `asm::ast`：
...
- [ ] `asm::parser`：手写递归下降，覆盖 M2~M6 指令所需语法。
- [ ] **伪指令**：`segment/ends`、`assume`、`db/dw/dd`、`dup`、`offset`、`end <label>`、`org`（教材偶尔用）。
- [ ] `asm::diagnostics`：`Span { line, col, len }`，错误信息能定位到列。
- [ ] 用 `insta` 对 5~10 个教材风格 fixture 做 AST 快照测试。
