...
- [x] `vm::i8086::memory::Memory`：1 MiB 线性数组；`phys(seg, off) = seg << 4 + off`；`read/write u8/u16` + 越界错误（教材会绕回，但默认报错可在 `--lenient` 下降级为绕回，方便排错）。
- [x] `vm::i8086::exec::Vm`：组合 cpu+memory+program；`step()` 返回 `StepOutcome { effects, next_ip, halted }`。
- [x] **段加载器**：按 `segment` 顺序铺到内存；`end start` 决定入口段:偏移；`assume` 仅记录辅助信息，不参与执行（教材语义）。
...
```
┌─ vasm  examples/ch10_call_ret.asm   ● Paused   cs:ip=076A:0012   #steps=42  x1 ─┐  ← 1. 状态栏（1 行）
├──────────────────────┬──────────────────────┬────────────────────────────────┤
...
- [x] `app::App`：持有 `Vm` 与渲染所需投影状态；`tick/event` 双循环。
- [x] `ui::panes::status`：顶部状态栏。字段：文件名、运行状态（Running/Paused/Halted/Error）、`cs:ip`、已执行指令计数、运行倍速、**当前焦点面板名**（防止用户在 Console 焦点下误按控制键）。
- [x] `ui::panes::source`：**只读**查看器；语法高亮（关键字、寄存器、立即数、注释、字符串、标签）；当前 `cs:ip` 行 `▶` 标记；断点行 `●` 标记。按 `e` 调起外部编辑器（回退链：Unix `$EDITOR → $VISUAL → vi`；Windows `%EDITOR% → %VISUAL% → notepad.exe`），退出后自动重载并重置 VM。GUI 编辑器需要用户自行在环境变量里加 `--wait` 之类参数，README 给提示。
- [x] `ui::panes::console`：DOS 字符输出缓冲（M5 才会有真实写入）；获得焦点时所有按键作为程序输入，**焦点状态在状态栏显式可见**。
- [x] `ui::panes::registers`：通用寄存器 + ip；变化的寄存器高亮一次。
- [x] `ui::panes::segments`：cs/ds/ss/es 及其当前指向段名。
- [x] `ui::panes::flags`：CF/PF/AF/ZF/SF/TF/IF/DF/OF 状态点。
- [x] `ui::panes::stack`：以 `ss:sp` 为基准上下展示 N 个 word，`sp` 指向位置高亮。
- [x] `ui::panes::call_stack`：逻辑调用栈（M3 阶段先占位渲染，真正维护在 M4 `call/ret` 实现时补齐）。
- [x] `ui::panes::memory`：横向独占一行，可指定 `seg:off` 起点的 hex+ASCII dump（默认 `ds:0`）；支持上下滚动；`g` 跳转到地址。
- [x] `ui::panes::explain`：底部第 1 行，显示即将执行指令的等价伪代码 + 数值代入 + 即将变化的 flags（M3 用占位文本，元数据驱动在 M5/M6 完善）。
- [x] `ui::panes::keymap`：底部第 2 行，根据当前焦点显示对应键位（Console 焦点下只显示"Esc 退出输入"）。
...
- [x] 算术/逻辑：`mul`、`div`、`and`、`or`、`xor`、`not`、`neg`、`shl`、`shr`、`sal`、`sar`、`rol`、`ror`、`rcl`、`rcr`
- [x] 比较：`cmp`、`test`
- [x] 跳转：`jmp short/near/far`、`jmp word ptr ds:[...]`（教材 §16 会用）、`jcxz` — `jmp word ptr ...` 留到 M6
- [x] 条件跳转：`je/jne/jz/jnz/jg/jge/jl/jle/ja/jae/jb/jbe/jo/jno/js/jns/jp/jnp`
- [x] 过程：`call`（near/far）、`ret`、`retf`
...
- [x] `b` 在源码光标处设/取消断点
- [x] `r` 重置 VM 到初始状态
- [x] `g` 跳到 `seg:off` 或标签（弹出输入框）
- [x] `Tab` 切换焦点面板；`↑↓PgUp/PgDn` 在焦点面板内滚动
- [x] 断点容器；执行循环中检查命中。
- [x] 源码区显示断点标记 `●` 与当前指令 `▶`。
...
- [x] `int` 指令：识别中断号并查表分派到 stub；保留中断向量表的逻辑结构（位于 `0:0`），允许程序读写但 stub 路径不实际解引用。
- [x] `iret`：仅在用户自定义中断处理程序里有意义；M5 阶段实现完整压栈/弹栈语义。
- [x] `vm::dos`：`int 21h` 至少实现教材使用到的功能号
  - `01h` 键盘输入回显、`02h` 显示单字符、`09h` 显示 `$` 结尾字符串、`0Ah` 缓冲键盘输入
  - `4Ch` 退出（驱动 VM 停机）
- [x] **DOS 字符编码**：VM 内部一律按字节存（不假设编码）；Console 面板渲染与输入时做编码转换。CLI 增加 `--encoding utf8|gbk|cp437`，默认 `gbk`（与王爽教材中文字符串期望一致）。
- [x] `vm::bios`：
  - `int 10h`：`00h` 设置显示模式（仅记录）、`02h` 设置光标位置、`09h/0Ah` 字符输出、`13h` 字符串输出
  - `int 16h`：`00h/01h` 键盘读取（从 TUI 输入缓冲取）
- [x] `in`、`out`：教材 §14 用到，但仅对少数已知端口 stub（如 60h 键盘扫描码），其他端口报"未实现"。
- [ ] 外中断（§15）：以"事件注入"形式支持，TUI 可手动触发一次特定 IRQ；不做真实异步。（M5 仅维护 IF；事件注入延后到 M6）
- [x] `cli`、`sti`：维护 IF 标志，不影响 stub 路径。
...
- [x] 直接定址表（§16）：`jmp word ptr ds:[bx+...]`、`call word ptr ...` 的间接跳转/调用，依托 M4 已实现的语法但要补 fixture。
- [x] BIOS 键盘与磁盘（§17）：`int 16h` 高级功能、`int 13h` 仅 stub 到读扇区的最小语义（提供一个虚拟 1.44MB 磁盘镜像参数 `--disk a.img`）。
- [x] 执行历史：每步保存 diff（被改寄存器/内存的旧值）；`u` 回退一步、`U` 回退到上一断点。
- [x] 观察点 (watchpoint)：对寄存器或 `seg:off` 设观察点，值变化时自动暂停。
- [x] 指令面板：在源码下方显示即将执行指令的语义说明（自然语言 + 等价 C 伪代码），从 `vm/i8086/isa/` 元数据生成。
- [x] 诊断浮层：执行错误（越界、未实现指令、除零）以模态框展示，回车关闭并保留状态。
- [x] 简易调用栈：根据 `call/ret` 维护逻辑调用栈，单独面板（按 `Tab` 可切到，或 F4 直达）。
- [x] 主题/配色可在 `~/.config/vasm/config.toml` 中覆盖。

---

## M7 · v1 发布

- [x] README：截屏、安装、教程示例、章节对照表。
- [x] `cargo install --path .` 一键安装通过。
- [x] `examples/` 按学习曲线组织（`chXX_*.asm`），覆盖 §1~§14、§16~§17。
- [ ] 性能基线（v1 范围外，留到 v2 视情况补）。
- [ ] v0.1.0 tag + GitHub Release（用户手动收尾）。
- [x] GitHub Actions CI（待 Stage E 落地）。

---

## v2 路线（x86-64，发布后启动）

详见 [`plan_v2.md`](./plan_v2.md)。本文档不再追加 v2 相关条目，避免内容漂移。
